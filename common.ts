...
import { readableStreamToString, TremendousError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TremendousError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TremendousError(e, e.response?.data)
                }
                if (innerError instanceof TremendousError) {
                    // Got "TremendousError" from the above try block
                    throw innerError;
                }
